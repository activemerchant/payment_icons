...
class PaymentIconTest < ActiveSupport::TestCase
  ...
  test 'Payment icon SVGs with <img> or <image> tags base64-encode PNG image data' do
    SVG_PAYMENT_TYPES.each do |_payment_type, svg|
      document = Nokogiri::XML.parse(svg)
  ...
  test 'find_icon returns the default icon when no variant is requested' do
    default_payment_icon_path = 'payment_icons/visa.svg'
    File.stubs(:exist?).with(default_payment_icon_path).returns(true)
    assert_equal 'payment_icons/visa.svg', PaymentIcon.find_icon('Visa')
  end

  test 'find_icon returns correct variant if available' do
    # icon_name = 'visa'
    # variant = 'bordered'
    payment_icon_path = 'payment_icons/visa_bordered.svg'
    # default_payment_icon_path = 'payment_icons/visa.svg'

    File.stubs(:exist?).with(payment_icon_path).returns(true)
    assert_equal payment_icon_path, PaymentIcon.find_icon('visa', 'bordered')
  end

  test 'find_icon returns default path if the variant is valid but asset file is not available' do
    payment_icon_path = 'payment_icons/visa_bordered.svg'
    default_payment_icon_path = 'payment_icons/visa.svg'

    File.stubs(:exist?).with(payment_icon_path).returns(false)
    File.stubs(:exist?).with(default_payment_icon_path).returns(true)
    assert_equal default_payment_icon_path, PaymentIcon.find_icon('visa', 'bordered')
  end

  test 'find_icon falls back to default icon if the requested variant is missing' do
    default_payment_icon_path = 'payment_icons/master.svg'
    File.stubs(:exist?).with(default_payment_icon_path).returns(true)
    assert_equal 'payment_icons/master.svg', PaymentIcon.find_icon('master', 'bordered')
  end

  test 'find_icon raises an error for an unsupported variant' do
    assert_raises ArgumentError do
      PaymentIcon.find_icon('visa', 'random_variant')
    end
  end

  test 'find_icon is case insensitive for icon names' do
    default_payment_icon_path = 'payment_icons/visa.svg'
    File.stubs(:exist?).with(default_payment_icon_path).returns(true)
    assert_equal 'payment_icons/visa.svg', PaymentIcon.find_icon('visa')
    assert_equal 'payment_icons/visa.svg', PaymentIcon.find_icon('VISA')
