...
class IconsControllerTest < ActionController::TestCase
  ...
  test 'show page has digested image tags for all credit card icons' do
    ...
    PaymentIcon.credit_cards.each do |card|
      assert_select "##{card.name}" do |_element|
        assert_select '[src=?]',
                      "/assets/#{Rails.application.assets.find_asset("payment_icons/#{card.name}.svg").digest_path}"
      end
    end
  end
  ...
  test 'show page has digested image tags for all cryptocurrency icons' do
    ...
    PaymentIcon.cryptocurrencies.each do |currency|
      assert_select "##{currency.name}" do |_element|
        assert_select '[src=?]',
                      "/assets/#{Rails.application.assets.find_asset("payment_icons/#{currency.name}.svg").digest_path}"
      end
    end
  end
end
